"""Portable V68 model probe; Python standard library only."""
import argparse
import hashlib
import json
import math
...
def verify_bundle():
    ...
    if manifest['htp_architecture'] != 68:
        raise ValueError('清单架构不是 V68。')
    listed = set()
    ...
    print(f'[PASS] V68 本地资源 SHA-256 校验通过：{len(listed)} 个文件；不代表实机验证通过。')
...
def main():
    parser = argparse.ArgumentParser(description='V68 HTP 模型推理及 CPU 输出对比')
    ...
    try:
        ...
        required = ['runtime/bin/qnn-platform-validator',
                    ...
                    'runtime/dsp/libQnnHtpV68Skel.so', 'inputs/input_list.txt',
                    'inputs/1x299x299x3_float_1.raw', 'inputs/1x299x299x3_float_2.raw']
        required += ['runtime/lib/' + name for name in
                     ['libQnnHtp.so', 'libQnnCpu.so', 'libQnnHtpPrepare.so', 'libQnnHtpV68Stub.so']]
        for name in required:
            if not (ROOT / name).is_file():
                raise FileNotFoundError(f'缺少随包文件：{name}')
        print('[1/6] 检查设备连接（本包适用 HTP V68）...')
        ...
        if not re.search(r'Hexagon Architecture V68\b', architecture):
            raise RuntimeError('设备架构不是 V68 或未能确认；停止模型推理。')
        ...
        if logs.exists():
            save('result.json', dict(Passed=code == 0, ExitCode=code, Device=serial, Remote=REMOTE, ExpectedArchitecture='V68'))
