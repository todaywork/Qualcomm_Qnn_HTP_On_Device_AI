...
def main():
    ...
    def adb(name, *command, show=False):
        result = subprocess.run([str(ROOT / 'tools/adb/adb.exe'), *selection, *command],
    ...
    try:
        ...
        required = ['tools/adb/adb.exe', 'tools/adb/AdbWinApi.dll', 'tools/adb/AdbWinUsbApi.dll',
        ...
        if logs.exists():
            save('result.json', dict(Passed=code == 0, ExitCode=code, Device=serial, Remote=REMOTE))
